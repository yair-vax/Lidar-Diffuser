...
class Stgl_Sml_GauDiffusion_InvDyn_V1(nn.Module):
    def __init__(self, model: Unet1D_TjTi_Stgl_Cond_V1, 
        ...
        self.model = model
        ## width of the goal "band": inpaint the last K trajectory frames as the
        ## goal sequence (K=1 == original single goal-frame behavior).
        self.goal_band_k = int(diff_config.get('goal_band_k', 1))
        ## STRIDE (in trajectory frames) between goal-band frames. The training data is
        ## dense (~0.03 cell/frame), so contiguous frames are near-identical; a stride
        ## makes the K goal scans span real distance (a proper multi-scan goal). MUST
        ## match the dataset (get_conditions) and the eval band step. 1 == original.
        self.goal_band_stride = max(int(diff_config.get('goal_band_stride', 1)), 1)
    ...
    def get_tj_cond(self, x, g_cond, timesteps):
        ...
        if g_cond['do_cond'] == 'both_ovlp': ## full_clean
            ...
            x, tj_cond = self.create_eval_tj_cond(
                ...
                t_type=g_cond['t_type'],
                is_noisy=False,
                stgl_cond={ _kk: _vv for _kk, _vv in g_cond['stgl_cond'].items() if _kk != 0 },
                )
            tj_cond['do_cond'] = True
    ...
    def p_losses(self, x_start, x_noisy, noise, t_2d, tj_cond, mask_no_noise=None, batch_loss_w=None,):
        ...
        if self.w_loss_type == 'no_unused':
            ...
            ## no loss on ANY inpainted goal-band frame (strided: H-1, H-1-S, H-1-2S, ...)
            _gbs = max(int(getattr(self, 'goal_band_stride', 1)), 1)
            for _kk in range(self.goal_band_k):
                _gi = max(self.horizon - 1 - _kk * _gbs, 0)
                batch_loss_w[tj_cond['is_end_inpat'], _gi] = 0.
            # pdb.set_trace()
    ...
    def create_train_tj_cond(self, x_clean: torch.Tensor, 
        ...
        ####
        ## goal band: every non-start key in cond_st_gl is a goal-band frame
        _goal_keys = [ _kk for _kk in cond_st_gl.keys() if _kk != 0 ]
        cond_end = { _kk: cond_st_gl[_kk][end_cd_use_inpat] for _kk in _goal_keys }
    ...
    def create_eval_tj_cond(self, 
        ...
        if end_traj == None:
            ...
            assert all(_kk == 0 for _kk in stgl_cond.keys())  ## no goal-band keys when end-overlap used

        ## do inpainting conditioning
        _goal_keys = [ _kk for _kk in stgl_cond.keys() if _kk != 0 ]   ## goal band (>= 1 frame)
        if len(_goal_keys) > 0:
            # pdb.set_trace()
            assert end_traj is None
            end_cond = { _kk: stgl_cond[_kk] for _kk in _goal_keys }
        ...
        assert ( (st_traj is not None) or 0 in stgl_cond ) and \
                    ( (end_traj is not None) or any(_kk != 0 for _kk in stgl_cond) )
