...
class OgB_Stgl_Sml_MazeEnvPlanner_V1:
    ...
    def setup_load(self, ld_config):
        ...
        ## (the LiDAR subclass overrides _setup_normalizers to load the real
        ##  LiDAR dataset normalizers instead of the hard-coded (x, y) ones.)
    ...
    def create_env(self):
        ...
        if Is_OgB_Robot_Env:
            ...
            raise NotImplementedError
        ## goal_tol override (2026-07-22): env._goal_tol is a plain instance
        ## attribute (0.5mj for non-point mazes, see ogbench maze.py), read fresh
        ## at every success check (np.linalg.norm(get_xy()-cur_goal_xy) <=
        ## _goal_tol) -- overriding it here loosens what counts as "touching the
        ## goal" without touching the vendored ogbench package at all.
        goal_tol_override = getattr(self.args, 'goal_tol_override', None)
        if goal_tol_override is not None:
            old_tol = self.env._goal_tol
            self.env._goal_tol = float(goal_tol_override)
            utils.print_color(
                f'[setup_load] goal_tol override: {old_tol} -> {self.env._goal_tol}', c='y')
    ...
    def load_inv_model(self):
        ...
        ## v2 inv-dyn variants (2026-07-07): auto-detect the goal encoding and
        ## action-chunk size from the RUN'S OWN trainer pkl, so switching
        ## checkpoints via LIDAR_INV_MODEL_PATH can never silently feed a
        ## diff-trained model absolute goals (the wrong-checkpoint class of
        ## error that burned the widek probe). Defaults = legacy, bit-exact.
        self._inv_goal_enc = 'abs'
        self._inv_act_chunk = 1
        try:
            from diffuser.utils import load_config
            _tr = load_config(self.inv_model_path, 'inv_trainer_config.pkl')
            _td = _tr._dict.get('trainer_dict', {}) or {}
            self._inv_goal_enc = str(_td.get('goal_encoding', 'abs'))
            self._inv_act_chunk = int(_td.get('act_chunk', 1) or 1)
        except Exception:  # noqa: BLE001
            pass
        self._chunk_buf = []
        self._chunk_repl = None
        _cx = int(getattr(self.args, 'act_chunk_exec', 0) or 0)
        self._act_chunk_exec = _cx if _cx > 0 else max(self._inv_act_chunk // 2, 1)
        if self._inv_goal_enc != 'abs' or self._inv_act_chunk > 1:
            utils.print_color(
                f'[inv v2] goal_encoding={self._inv_goal_enc} '
                f'act_chunk={self._inv_act_chunk} (exec {self._act_chunk_exec}/query)', c='y')
        ...
        ** OGBench **
        ...
        for i_et in range(self.n_max_steps):
            ...
            if self.act_control == 'pred_inv':
                ...
                ## v2 inv-dyn variants are wired only into the main replan loop;
                ## fail loudly rather than silently mis-encode goals here.
                assert getattr(self, '_inv_goal_enc', 'abs') == 'abs' and \
                    getattr(self, '_inv_act_chunk', 1) == 1, \
                    'diff/chunk inv-dyn not supported in ogb_env_interact_1_ep'
    ...
    def ogb_plan_once(self, pl_seed=None, given_probs=None): ## num_ep
        ...
        ep_cnt_repls = []
        ep_cnt_env_steps = []
        ## --- goal-distance logging (additive; agent xy vs goal, mj units = the success metric) ---
        ep_min_goal_dist = []    ## min over steps of ||get_xy() - cur_goal_xy||
        ep_final_goal_dist = []  ## distance to goal at the last step
        ep_argmin_step = []      ## step index of closest approach
        ep_repl_goal_dists = []  ## per-ep list of [i_et, dist_to_goal] at each replan
        ...
        for i_ep in range(self.ep_st_idx, self.ep_st_idx+num_ep):
            ...
            if hasattr(self, '_lidar_arrival_reset'):
                self._lidar_arrival_reset(gl_pos)   ## PM: reset LiDAR self-loc + goal-arrival for this episode
            ...
            if self.is_replan == 'at_given_t':
                ...
                ## LiDAR periodic replan (added): big fused buffer; we replan
                ## whenever the current plan is consumed (see trigger below). The
                ## composition count stays full each time -> fresh full-horizon plan.
                self.max_n_repl = self.repl_ada_dist_cfg.get('max_n_repl', 15)
                ## repl_every>0: also fire a replan every N consumed waypoints
                ## (not only when the whole plan is used up). 0 = original behavior.
                self.repl_every = self.repl_ada_dist_cfg.get('repl_every', 0)
                tot_hzn = 10000
            ...
            ep_step_goal_dists = []  ## dist(agent_xy, goal) at each env step (this ep)
            repl_goal_dists = []     ## [i_et, dist] at each replan (this ep)
            ...
            for i_et in range(self.n_max_steps):
                ## get current obs
                obs_cur = self.env.get_ob().copy()
                ## the observation fed to the inverse-dynamics model (overridable:
                ## LiDAR uses the 34-D [x,y,cos,sin,lidar] composite here)
                obs_cur_nm = self._get_invdyn_obs_nm(obs_cur)
                ...
                ## NOTE: only the 'ada_dist' replan strategy reads this distance and
                ## the self.ada_dist_* attributes; guard it so the no-replan path
                ## (e.g. the LiDAR rollout, is_replan=False) does not touch them.
                if i_et > 0 and self.is_replan == 'ada_dist':
                ...
                elif self.is_replan == 'periodic' and i_et > 0:
                    ## replan when the current plan is consumed, OR (if repl_every>0)
                    ## every repl_every consumed waypoints since the last plan -- so the
                    ## ant re-aims more often than once per ~full-horizon plan.
                    _wp_since_plan = wp_idx - prev_dfu_wp_idx
                    _consumed = _wp_since_plan >= len(all_plan_trajs_ep[-1])
                    _periodic = (self.repl_every > 0) and (_wp_since_plan >= self.repl_every)
                    ## verify-and-hop (LiDAR): a rejected-decoy hop asks for a prompt
                    ## re-route to the next posterior hypothesis. Absent attr -> False,
                    ## so non-LiDAR rollouts are bit-for-bit unchanged.
                ...
                if i_et == 0 or (is_do_repl_et and self.is_replan):
                    ## a plan is being (re)generated -> the verify-and-hop re-route
                    ## request (if any) is now satisfied; clear it.
                    self._nav_force_replan = False
                    ## run diffusion model
                    ## pick obs_dims for diffusion planner (overridable: LiDAR
                    ## replaces these with the start scan and the goal scan)
                    input_st, gl_pos_for_dfu = self._get_planner_stgl_input(obs_cur, gl_pos)

                    # pdb.set_trace()

                    ## start (1 row) + goal band (1 row for a single goal, K rows
                    ## for a multi-scan goal) -> (1+K, n_probs=1, dim). K=1 is
                    ## bit-for-bit the original (2, 1, dim) [start, goal] stack.
                    _st_arr = np.atleast_2d(np.asarray(input_st, dtype=np.float32))
                    _gl_arr = np.atleast_2d(np.asarray(gl_pos_for_dfu, dtype=np.float32))
                    g_cond = {
                        ## (1+K, 1, dim), not normalized
                        'st_gl': np.concatenate([_st_arr, _gl_arr], axis=0)[:, None, :].astype(np.float32)
                    }

                    if is_do_repl_et: ## this is doing a replanning
                        cnt_repl += 1
                        ## how far from goal (xy) is the agent when this replan fires
                        repl_goal_dists.append([i_et, float(np.linalg.norm(self.env.get_xy() - self.env.cur_goal_xy))])
                        if self.is_replan == 'at_given_t':
                ...
                if self.act_control == 'pred_inv':
                    ## gridnav pursuit (guarded; inert unless the LiDAR nav planner
                    ## defines it): closed-loop waypoint progression -- the subgoal
                    ## advances with the ant's LiDAR-localized position instead of the
                    ## wall clock, so the target scan cannot race ahead of a slow ant.
                    if hasattr(self, '_nav_pick_wp_idx'):
                        wp_idx = self._nav_pick_wp_idx(wp_idx, len(fused_traj_ep))
                    ...
                    obs_cur_nm = utils.to_torch(obs_cur_nm)[None,]
                    goal_cur_nm = utils.to_torch(goal_cur_nm)[None,]
                    _gd = goal_cur_nm.shape[-1]
                    ## v2a difference-goal encoding (auto-detected from the inv
                    ## run's trainer pkl in load_inv_model; 'abs' = legacy path).
                    ## goal input = normed(goal scan) - normed(current scan); the
                    ## current scan is the trailing goal-dim slice of the inv obs.
                    if getattr(self, '_inv_goal_enc', 'abs') == 'diff':
                        goal_in_nm = goal_cur_nm - obs_cur_nm[..., -_gd:]
                    else:
                        goal_in_nm = goal_cur_nm
                    if getattr(self, '_inv_act_chunk', 1) > 1:
                        ## v2b action-chunk executor (receding horizon): query the
                        ## model for m actions, play the first act_chunk_exec of
                        ## them open-loop, re-query. Buffer is flushed on replan
                        ## and at episode start (i_et==0) so a stale chunk never
                        ## outlives its plan. act-CFG is intentionally skipped
                        ## for chunk models (goal usage is trained-in, not
                        ## amplified). NOTE act clipped to [-1,1] normed bounds.
                        if i_et == 0 or self._chunk_repl != (i_ep, cnt_repl):
                            self._chunk_buf = []
                            self._chunk_repl = (i_ep, cnt_repl)
                        if not self._chunk_buf:
                            _ck = self.inv_model(obs_cur_nm, goal_in_nm)
                            _ck = _ck.cpu().numpy()[0].reshape(self._inv_act_chunk, -1)
                            self._chunk_buf = list(_ck[:self._act_chunk_exec])
                        act_pred_nm = np.clip(self._chunk_buf.pop(0), -1.0, 1.0)
                    else:
                        ## note that the act_pred is not clipped, e.g., might be > 1.0
                        act_pred_nm = self.inv_model(obs_cur_nm, goal_in_nm).cpu().numpy()[0]
                        ## goal-contrast guidance (LiDAR planner sets _act_cfg_w; default
                        ## 1.0 = block skipped, bit-exact legacy). Amplifies the measured-
                        ## weak goal channel: a = a_self + w*(a_goal - a_self), a_self =
                        ## the action under the "stay here" goal (current scan for 'abs',
                        ## the ZERO diff for 'diff'). Clip in normalized space.
                        _w = float(getattr(self, '_act_cfg_w', 1.0) or 1.0)
                        if _w != 1.0:
                            if getattr(self, '_inv_goal_enc', 'abs') == 'diff':
                                _g_self = torch.zeros_like(goal_in_nm)
                            else:
                                _g_self = obs_cur_nm[..., -_gd:]
                            act_self_nm = self.inv_model(obs_cur_nm, _g_self).cpu().numpy()[0]
                            act_pred_nm = act_self_nm + _w * (act_pred_nm - act_self_nm)
                            act_pred_nm = np.clip(act_pred_nm, -1.0, 1.0)
                ...
                obs_cur, rew, terminated, truncated, info = self.env.step(act_pred)
                ## distance to goal using the SAME xy the success check uses (env.step line ~510)
                ep_step_goal_dists.append(float(np.linalg.norm(self.env.get_xy() - self.env.cur_goal_xy)))

                is_suc = bool(info['success']) or is_suc ## sparse reward

                ## PM Stage-3: LiDAR self-localization + goal-arrival. The ant ends the run
                ## when its scan matches the goal grid pose. Terminates WITHOUT faking env
                ## success -> is_suc (hence ep_srate) stays ground-truth; a break with
                ## is_suc=False is a LiDAR false-positive we can then measure.
                if hasattr(self, '_lidar_arrival_step'):
                    _arr = self._lidar_arrival_step(obs_cur)
                    if _arr is not None and _arr.get('arrived') and getattr(self, '_arrival_ends', False):
                        utils.print_color(
                            f'[arrival] end ep {i_ep} @step {i_et}: LiDAR goal match '
                            f'(env is_suc={is_suc})', c='g')
                        break
                ...
                if i_et % 100 == 0 and i_et != 0:
                    tmp_dist_1 = self._get_subgoal_log_dist(obs_cur, goal_cur)
            ...
            ## --- aggregate goal-distance stats for this episode ---
            _gd = np.asarray(ep_step_goal_dists, dtype=np.float64)
            if _gd.size == 0:
                _gd = np.array([np.linalg.norm(self.env.get_xy() - self.env.cur_goal_xy)])
            ep_min_goal_dist.append(float(_gd.min()))
            ep_argmin_step.append(int(_gd.argmin()))
            ep_final_goal_dist.append(float(_gd[-1]))
            ep_repl_goal_dists.append(repl_goal_dists)
        ...
        json_data.update([
            ...
            ## --- goal-distance diagnostics (mj units; success iff min dist <= goal_tol) ---
            ('goal_tol', float(self.env._goal_tol)),
            ('ep_min_goal_dist', [round(float(x), 3) for x in ep_min_goal_dist]),
            ('ep_final_goal_dist', [round(float(x), 3) for x in ep_final_goal_dist]),
            ('ep_argmin_step', ep_argmin_step),
            ('ep_repl_goal_dists', ep_repl_goal_dists),
    ...
    ## ------------------------------------------------------------------
    ## Overridable hooks. The base behaviour is identical to the original
    ## inline (x, y) code; the LiDAR planner subclass overrides them so the
    ## planner is conditioned on scans and the inv-dyn receives the 34-D
    ## [x, y, cos, sin, lidar] observation. See LIDAR_ROLLOUT_VIDEO_TODO.md.
    ## ------------------------------------------------------------------
    def _setup_normalizers(self):
        '''Build the (train, full) normalizers used at rollout time.
        Base: hard-coded per-dataset (x, y) normalizer values.'''
        args_train = self.args_train
        self.train_normalizer = utils.load_ogb_maze_datasetNormalizer(args_train,)
        ## for inv dyn model (full obs + action stats)
    ...
    def _get_planner_stgl_input(self, obs_cur, gl_pos):
        '''Start/goal vectors used to condition the diffusion planner.
        Base: the obs_select_dim slice of the raw obs / goal (e.g. x, y).'''
        return obs_cur[self.obs_select_dim,], gl_pos[self.obs_select_dim,]

    def _get_invdyn_obs_nm(self, obs_cur):
        '''Normalized observation fed to the inverse-dynamics model.
        Base: the (normalized) raw env obs.'''
        return self.full_normalizer.normalize(obs_cur, 'observations')

    def _get_subgoal_log_dist(self, obs_cur, goal_cur):
        '''A scalar progress metric (current obs vs current subgoal) for logging.'''
        return np.linalg.norm(obs_cur[self.obs_select_dim,] - goal_cur).item()
