...
class OgB_ReplayBuffer_V2:
    """Dec 29 init from diffuser/datasets/buffer.py"""
    ...
    def get_pad_len(self, path: dict):
        ...
        ## seems like a general Padding method, check back and remove later
    ...
    def pad_path_with_last(self, path: dict):
        ...
        for key in path.keys():
            ...
            elif self.pad_type == 'first':
                ## Dec 29 New
                new_path[key] = np.concatenate( [elem_first]*pad_len + [elem,] , axis=0 )
            else:
                assert False
    ...
    def add_path(self, path):
        if self.pad_type in ['last', 'first_last', 'first'] and self.use_padding:
